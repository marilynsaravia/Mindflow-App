...
export default function Welcome() {
  ...
  const handleStart = () => {
    ...
    router.push(`/quiz?name=${encodeURIComponent(trimmedName)}`);
  };

  const isDisabled = !name.trim() || !agree;
    ...
    <main className="w-full min-h-screen bg-[#0E0F0F] flex items-center justify-center px-4">
      <div className="w-full max-w-7xl flex flex-col md:flex-row items-center justify-between gap-12">
        <section className="w-full md:max-w-xl text-center md:text-left">
          <img className="mb-10" width="100" height="100" src="/logo.svg" alt="Mindflow logo" />
          ...
          <p className="text-white text-md md:text-lg leading-relaxed mb-12">
            Enter your name so we can personalize your experience and guide you through the test gently.
          </p>
        </section>

        <section className="min-w-lg flex items-center justify-center p-4 w-full md:w-auto">
          <div className="flex flex-col bg-white w-full md:w-[420px] h-auto rounded-xl p-9 shadow-2xl">
            {/* Welcome Title */}
            ...
            <div className="flex items-start mb-6">
              <input
                ...
                className="mt-1 w-5 h-5 border-gray-300 rounded focus:ring-[#27D89D] accent-[#27D89D]"
              />
              <label htmlFor="agree" className="ml-3 text-sm text-gray-600">
                I agree to all the
                <a href="#" className="text-[#36b68a] hover:text-[#27D89D] font-medium">
                  {" "}
                  Terms
                </a>
                ,{" "}
            ...
            <button
              ...
              className="
                ...
                disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-[#27D89D]
              "
            >
              Start test
            </button>
